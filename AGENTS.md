...
- **Orchestrator needs ClusterRole** (not Role): It creates tenant namespaces (`Codex-t-{id}`) and spawns pods across them.
...
- **Cloudflared is standalone**: Separate deployment in `Codex-runner` namespace pinned to dell-server — NOT a sidecar of the orchestrator. Defined in `cassandra-k8s/apps/cloudflared/`.
...
- **Tool deferral bites MCP**: Codex CLI defers ALL MCP tools behind `ToolSearch` by default. Set `ENABLE_TOOL_SEARCH=false` in runner child env to load tools eagerly.
...
- **Multiple GPU nodes**: Each needs unique WSL MAC in `.wslconfig` to avoid DHCP collision. See `.Codex/rules/infra-context.md` for IPs and SSH details.
